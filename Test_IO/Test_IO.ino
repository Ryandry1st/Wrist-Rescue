 ...
 * Test_IO.c
 *
 *  Created on: Oct 9th, 2018
 *      Author: dreifuerstrm
 *
 *  Activates each of the IO options and shows that the hardware is working as expected
 *  Where interrupts are necessary, like the buttons and 3 axis data line, they are used.
 *  Otherwise functions are used to test features
 *
...
float xmax = 0;    //holds the max zvalue for fall test
float ymax = 0;    //holds the max zvalue for fall test
...
void i2c_com() {
  ...
  if (abs(ymax) < abs(y_g)) {    //calculate zmax to put on serial monitor
    ymax = abs(y_g);
  }
  if (abs(xmax) < abs(x_g)) {    //calculate zmax to put on serial monitor
    xmax = abs(x_g);
  }
